...
本文档面向需要对接此后端API的前端开发者，提供实用的代码示例和最佳实践。
...
# 生产环境（.env.production）建议使用相对路径以解决跨域问题：
# VITE_API_BASE_URL=/
```
...
└── views/
    ├── Login.vue             # 登录页
    ├── student/
    │   ├── AchievementSubmit.vue    # 成果提交
    │   ├── AchievementList.vue      # 我的成果
    │   └── AIChat.vue               # AI对话
    └── admin/
        └── AchievementReview.vue    # 成果审核
```
 ...
 * @param {string} data.type - 成果类型
 * @param {object} [data.content_json] - 详细信息
 * @param {string} [data.evidence_url] - 证书URL
...
/**
 * AI对话
 * @param {string|null} sessionId - 会话ID（新会话传null）
 ...
 * @param {string} [params.student_name] - 学生姓名搜索
 ...
 * @param {string} [comment] - 审核意见（拒绝时必填）
    ...
    <el-card>
      ...
      <div v-if="currentStep === 2" class="step-content">
        <el-result
          icon="success"
          title="提交成功"
          sub-title="您的成果已提交，等待审核"
        >
          <template #extra>
            <el-button type="primary" @click="$router.push('/student/achievements')">
              查看我的成果
            </el-button>
            <el-button @click="resetForm">继续提交</el-button>
          </template>
        </el-result>
      </div>
    </el-card>
...
<template>
  <div class="list-container">
    ...
    <!-- 详情对话框 -->
    <el-dialog v-model="dialogVisible" title="成果详情" width="600px">
...
**问题**: 开发环境前端请求后端出现CORS错误

**解决方案**: 
后端已配置CORS，确保前端URL在白名单中：
```env
# 后端 .env
